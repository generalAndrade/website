<?php
	...
	*/
 	...
 	/* CUSTOM CSS OUTPUT
 	================================================== */
 	if (!function_exists('iamaze_custom_styles')) { 
		function iamaze_custom_styles() {
			...
			*/
			...
			echo 'h1,h2,h3,h4,h5,h6,.comment-reply-title,.widget .widget-title {'.$tx_title_style.'}';				
			...
			echo '.iheader,.format-status {background-color: '.$primary_color.';}';
		...
		add_action('wp_head', 'iamaze_custom_styles');
	}
	...
	function nx_custom_script() {
		...
		$custom_js = $iamaze_data['custom_js'];
